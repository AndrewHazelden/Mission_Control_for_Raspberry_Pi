...
int main (int argc, const char * argv[]) {
  ...
  if (USF_error == 0){
    ...
    usf_read_int("view_layout", &view_layout );
...
void init_gfx(void){
  ...
  // 0=Vertical Block Layout, 1=Horizontal Layout 5up, 2=Horizontal Layout 6up
  if(view_layout == 0){
    // Tall mode - 2 wide x 3 tall gauges
    screen_width = DIAL_WIDTH*2;
    screen_height = DIAL_WIDTH*3;
  } else if(view_layout == 1){
    //Horizontal Gauge Strip Layout 5 Gauges
    screen_width = DIAL_WIDTH*6;
    screen_height = DIAL_WIDTH;
  } else if(view_layout == 2){
    //Horizontal Gauge Strip Layout 6 Gauges
    screen_width = DIAL_WIDTH*7;
    screen_height = DIAL_WIDTH;
  }
...
void swap_layout(void){
  ...
  if(view_layout == 0){
    ...
    heading_rect.x = 0;
    heading_rect.y = DIAL_WIDTH*1;

    airspeed_rect.x = DIAL_WIDTH*1;
    airspeed_rect.y = DIAL_WIDTH*1;

    altitude_rect.x = 0;
    altitude_rect.y = DIAL_WIDTH*2;

    variometer_rect.x = DIAL_WIDTH*1;
    variometer_rect.y = DIAL_WIDTH*2;
  } else if(view_layout == 1){
    //Horizontal Gauge Strip Layout 5 Gauges
    navputer_rect.x = 0;
    navputer_rect.y = 0;

    heading_rect.x = DIAL_WIDTH*2;
    heading_rect.y = 0;

    altitude_rect.x = DIAL_WIDTH*3;
    altitude_rect.y = 0;

    variometer_rect.x = DIAL_WIDTH*4;
    variometer_rect.y = 0;

    airspeed_rect.x = DIAL_WIDTH*5;
    airspeed_rect.y = 0;

    //The attitude display is unused in the 5 gauge display mode
    attitude_rect.x = 0;
    attitude_rect.y = DIAL_WIDTH*6;
...
void load_sprites(void){
  ...
  //Position the image
  //attitude_rect.x = DIAL_WIDTH*1;
  //attitude_rect.y = DIAL_WIDTH*2;
  ...
  //Set the view layout mode the first time
  swap_layout();  
}
...
void render_screen(void){
  ...
  //Only show the attitude gauges on when 6 gauges are visible
  if(view_layout == 2){
  ...
  */
  ...
  if(gprmc.mode[0] != 'N'){
    ...
    */
    ...
    //Write the screen dimensions in the KML file
    fprintf(kml_fp,"\t\t\t<size x=\"%d\" y=\"%d\" xunits=\"pixels\" yunits=\"pixels\"/>\n", screen->w, screen->h);

    //fprintf(kml_fp,"\t\t\t<size x=\"256\" y=\"384\" xunits=\"pixels\" yunits=\"pixels\"/>\n");
